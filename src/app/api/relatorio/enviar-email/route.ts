...
const EMAIL_DESTINO = process.env.RELATORIO_EMAIL_PARA || 'comercial@fertiflora.com'

/** Lê uma lista de e-mails separada por vírgula de uma env var, ignorando vazios. */
function lerListaEmails(valor: string | undefined): string[] {
  if (!valor) return []
...
function montarHtml(ordens: OrdemDiaria[], data: string): string {
  ...
  for (const o of ordens) {
    for (const item of o.itens ?? []) {
      ...
      for (const mp of MATERIAS_PRIMA) {
        ...
        if (kgPorTon > 0) consumoAcc[mp.key] = (consumoAcc[mp.key] ?? 0) + tons * kgPorTon
      }
    }
  }
  const consumo = MATERIAS_PRIMA
    .map((mp) => ({ mp, kg: consumoAcc[mp.key] ?? 0 }))
    .filter((x) => x.kg > 0)
    .sort((a, b) => b.kg - a.kg)
  ...
  const linhasOrdens = ordens.flatMap((o) => {
    ...
    return itens.map((item, idx) => {
      ...
      const usados = f
        ...
        : []
      const materiaTexto = usados.map((x) => `${x.mp.label} ${fmtKg(x.kg)}`).join(', ')
  ...
  const linhasConsumo = consumo.map(({ mp, kg }) => `
    <tr>
      <td style="padding:6px 8px;border-bottom:1px solid #e5e7eb;">${mp.label}</td>
...
export async function POST(req: NextRequest) {
  ...
  if (!apiKey) {
    return NextResponse.json(
      { error: 'Envio de e-mail não configurado. Peça ao administrador para configurar a RESEND_API_KEY.' },
      { status: 503 },
    )
  }
  ...
  const { data: ordens, error: dbError } = await (supabase as any)
    .from('ordens_diarias')
    .select(`
      *,
      itens:ordem_itens (
  ...
  if (dbError) {
    console.error('[relatorio/enviar-email] db', dbError)
    return NextResponse.json({ error: 'Erro ao carregar os dados do relatório.' }, { status: 500 })
  }
  ...
  if (!res.ok) {
    ...
    console.error('[relatorio/enviar-email] resend', errText)
    return NextResponse.json({ error: 'Erro ao enviar o e-mail. Tente novamente.' }, { status: 502 })
  }

  return NextResponse.json({ ok: true, para: EMAIL_DESTINO, cc })
}
