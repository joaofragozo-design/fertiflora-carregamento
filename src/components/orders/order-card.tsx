    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          {order.status === 'SOLICITADO' && onCancelar && (
            <button
              type="button"
          ...
          {order.status === 'SOLICITADO' && onIniciar && (
            <Button
              size="sm"
              variant="secondary"
              loading={loading}
          ...
          {order.status === 'EM_EXECUCAO' && onConcluir && (
            <Button
              size="sm"
              variant="primary"
              loading={loading}
              onClick={() => onConcluir(order)}
            >
              Finalizar
            </Button>
          )}
      ...
      <div className="mt-3 flex items-center gap-1.5 text-xs text-industrial-500">
        ...
        <span>
          {order.status === 'CONCLUIDO' && order.finished_at
            ...
            : order.status === 'EM_EXECUCAO' && order.started_at
