-- ============================================================
-- FERTI FLORA — Migration 071: Persiste o envio do WhatsApp da liberação
-- ============================================================
-- Bug: "Abrir WhatsApp" no painel de Solicitações só marcava um Set em
-- memória (useState) no componente — nada era gravado no banco. Ao navegar
-- pra outra tela (Solicitações virou rota própria na migration anterior) ou
-- recarregar a página, o agendamento LIBERADO reaparecia na fila como se o
-- WhatsApp nunca tivesse sido enviado. Agora o envio fica registrado na
-- própria linha e a query de pendências passa a excluir quem já foi enviado.

alter table public.programacao_carregamento
  add column if not exists whatsapp_enviado_em timestamptz;

-- Mesma trava de colunas da transportadora (migration 058): quem envia o
-- WhatsApp é sempre a Logística, então esse campo entra na lista de colunas
-- que a transportadora não pode alterar.
