...
OUTPUT_PATH = r"C:\Projetos\FertiFloraCarregamento\ORDEM_SERVICO_NR01_AUXILIAR_PRODUCAO.pdf"
...
def build_identificacao_colaborador():
    ...
    dados = [
        [Paragraph("<b>Nome:</b> " + "_" * 62, style_campo)],
        [linha_dupla("Cargo", "Auxiliar de Produção", "Setor", "Produção")],
...
def build_story():
    ...
    story.append(Spacer(1, 8))
    story.append(Paragraph(
        "Esta Ordem de Serviço tem por finalidade orientar o colaborador quanto às normas de "
        "Segurança e Saúde no Trabalho aplicáveis às atividades desenvolvidas na Fertiflora "
        "Fertilizantes Ltda., conforme a Norma Regulamentadora nº 01 (NR-01), estabelecendo os "
        "riscos ocupacionais, medidas preventivas, uso obrigatório de Equipamentos de Proteção "
        "Individual (EPIs) e responsabilidades durante a execução das atividades.", style_corpo,
    ))
    story.append(Spacer(1, 6))
    ...
    story.append(Spacer(1, 8))
    story.append(Paragraph(
        "O Auxiliar de Produção poderá executar, conforme necessidade operacional:",
        style_corpo,
    ))
    story.append(lista_marcadores([
        "Auxiliar nas operações das linhas de granulação;",
        "Auxiliar na linha de mistura de fertilizantes;",
        "Alimentar equipamentos e máquinas com matérias-primas;",
        "Auxiliar no ensaque dos produtos acabados;",
        "Organização e limpeza das áreas produtivas;",
        "Movimentação manual de materiais e embalagens;",
        "Auxiliar na operação das correias transportadoras;",
        "Auxiliar nas peneiras, elevadores e demais equipamentos do processo;",
        "Coletar resíduos e manter o ambiente limpo e organizado;",
        "Executar outras atividades correlatas determinadas pela supervisão.",
    ]))
    story.append(Spacer(1, 6))
    ...
    story.append(Spacer(1, 8))
    story.append(Paragraph(
        "O colaborador está sujeito, entre outros, aos seguintes riscos ocupacionais:",
        style_corpo,
    ))
    story.append(Paragraph("Riscos Físicos", style_subsecao))
    story.append(lista_marcadores([
        "Ruído proveniente dos equipamentos;",
        "Poeira de fertilizantes;",
        "Calor nas áreas próximas à fornalha e secador;",
    ...
    story.append(Spacer(1, 8))
    story.append(Paragraph("É obrigatório ao colaborador:", style_corpo))
    story.append(lista_marcadores([
        "Cumprir integralmente as normas internas da empresa;",
        "Utilizar corretamente todos os EPIs fornecidos;",
        "Nunca remover proteções de máquinas;",
        "Não operar equipamentos sem autorização;",
        "Comunicar imediatamente qualquer condição insegura;",
        "Manter o ambiente organizado;",
        "Respeitar sinalizações de segurança;",
        "Participar dos treinamentos promovidos pela empresa;",
        "Comunicar imediatamente qualquer acidente ou quase acidente.",
    ]))
    story.append(Spacer(1, 6))

    # 5. EQUIPAMENTOS DE PROTEÇÃO INDIVIDUAL (EPIs)
    story.append(secao_titulo("5. EQUIPAMENTOS DE PROTEÇÃO INDIVIDUAL (EPIs)"))
    story.append(Spacer(1, 8))
    story.append(Paragraph(
        "Durante a execução das atividades é obrigatório utilizar, conforme orientação da "
        "empresa:", style_corpo,
    ))
    story.append(lista_marcadores([
        "Botina de segurança;",
    ...
    story.append(secao_titulo("6. EM CASO DE EMERGÊNCIA"))
    story.append(Spacer(1, 8))
    story.append(Paragraph("O colaborador deverá:", style_corpo))
    story.append(lista_marcadores([
        "Interromper imediatamente a atividade quando houver risco à sua integridade física "
        "ou à de terceiros;",
        "Comunicar imediatamente o encarregado ou supervisor;",
        "Seguir as orientações da brigada de emergência, quando houver;",
        "Utilizar as rotas de fuga estabelecidas;",
        "Não retornar à área até autorização do responsável.",
    ]))
    story.append(Spacer(1, 6))

    # 7. RESPONSABILIDADES DO COLABORADOR
    story.append(secao_titulo("7. RESPONSABILIDADES DO COLABORADOR"))
    story.append(Spacer(1, 8))
    story.append(Paragraph("Declaro estar ciente de que:", style_corpo))
    story.append(lista_marcadores([
        "Recebi orientação sobre os riscos existentes em minha atividade;",
        "Recebi orientação sobre a utilização correta dos EPIs;",
        "Comprometo-me a cumprir todas as normas de segurança da empresa;",
        "Comprometo-me a comunicar qualquer situação de risco;",
        "Estou ciente de que o descumprimento das normas poderá acarretar medidas "
        "disciplinares previstas na legislação trabalhista e nas normas internas da empresa.",
    ]))
    story.append(Spacer(1, 6))

    # 8. BASE LEGAL
    story.append(secao_titulo("8. BASE LEGAL"))
    story.append(Spacer(1, 8))
    ...
    # 9. DECLARAÇÃO
    story.append(secao_titulo("9. DECLARAÇÃO"))
    story.append(Spacer(1, 8))
    story.append(Paragraph(
        "Declaro que recebi as orientações constantes nesta Ordem de Serviço, compreendi os "
        "riscos inerentes às minhas atividades, fui instruído quanto ao uso correto dos "
        "Equipamentos de Proteção Individual (EPIs) e comprometo-me a cumprir integralmente as "
        "normas de segurança estabelecidas pela Fertiflora Fertilizantes Ltda.", style_corpo,
    ))
    story.append(Spacer(1, 8))

    # 10. ASSINATURAS
    story.append(secao_titulo("10. ASSINATURAS"))
    story.append(Spacer(1, 10))
    assinaturas = Table(
        [[
            build_bloco_assinatura("Colaborador", [("Nome", None), ("Assinatura", None)]),
    ...
    story.append(Spacer(1, 6))
    story.append(Paragraph(
        "<i>Aprovação — Fertiflora Fertilizantes Ltda.</i>", style_assinatura_campo,
    ))
...
def footer(canvas, doc):
    ...
    canvas.drawString(20 * mm, 15 * mm, "FertiFlora Fertilizantes Ltda. — SST-OS-001 — Rev. 00")
...
def main():
    doc = SimpleDocTemplate(
        ...
        title="Ordem de Serviço NR-01 - Auxiliar de Produção",
