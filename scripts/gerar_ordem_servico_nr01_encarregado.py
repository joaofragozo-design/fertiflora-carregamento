...
OUTPUT_PATH = r"C:\Projetos\FertiFloraCarregamento\ORDEM_SERVICO_NR01_ENCARREGADO_PRODUCAO.pdf"
...
def build_identificacao_colaborador():
    ...
    dados = [
        [Paragraph("<b>Nome:</b> Diego Ianch Correa", style_campo)],
        [linha_dupla("Cargo", "Encarregado de Produção", "Setor", "Produção")],
...
def build_story():
    ...
    story.append(Spacer(1, 8))
    story.append(Paragraph(
        "Esta Ordem de Serviço tem por finalidade orientar o Encarregado de Produção quanto "
        "às normas de Segurança e Saúde no Trabalho aplicáveis às atividades desenvolvidas na "
        "Fertiflora Fertilizantes Ltda., conforme a Norma Regulamentadora nº 01 (NR-01), "
        "estabelecendo os riscos ocupacionais, medidas preventivas, uso obrigatório de "
        "Equipamentos de Proteção Individual (EPIs) e responsabilidades durante a execução "
        "das atividades.", style_corpo,
    ))
    story.append(Spacer(1, 6))
    ...
    story.append(Spacer(1, 8))
    story.append(Paragraph(
        "O Encarregado de Produção é responsável por coordenar, supervisionar e acompanhar "
        "as atividades da produção, podendo executar, entre outras, as seguintes atribuições:",
        style_corpo,
    ))
    story.append(lista_marcadores([
        "Coordenar as operações das linhas de granulação e mistura;",
        "Distribuir atividades aos colaboradores da produção;",
        "Acompanhar o desempenho dos equipamentos e do processo produtivo;",
        "Fiscalizar o cumprimento das normas de segurança e do uso correto dos EPIs;",
        "Acompanhar a operação de granuladores, secadores, peneiras, elevadores, correias "
        "transportadoras e demais equipamentos;",
        "Orientar os colaboradores quanto aos procedimentos operacionais;",
        "Comunicar irregularidades, falhas ou condições inseguras à gerência;",
        "Acompanhar serviços de manutenção quando necessário;",
        "Garantir a organização, limpeza e segurança das áreas produtivas;",
        "Auxiliar na investigação e comunicação de incidentes ou acidentes;",
        "Cumprir e fazer cumprir os procedimentos internos da empresa.",
    ]))
    story.append(Spacer(1, 6))
    ...
    story.append(lista_marcadores([
        "Exposição ao ruído industrial;",
        "Poeira proveniente da fabricação de fertilizantes;",
        "Calor nas áreas da fornalha e secador;",
        "Vibração de equipamentos.",
    ]))
    story.append(Paragraph("Riscos de Acidentes", style_subsecao))
    story.append(lista_marcadores([
        "Contato com máquinas em operação;",
        "Queda de materiais;",
        "Escorregamentos e tropeços;",
        "Esmagamentos;",
        "Queda ao mesmo nível;",
        "Circulação de empilhadeiras e equipamentos móveis.",
    ]))
    story.append(Paragraph("Riscos Ergonômicos", style_subsecao))
    story.append(lista_marcadores([
        "Caminhadas frequentes;",
        "Permanência prolongada em pé;",
        "Esforço físico eventual;",
        "Movimentação manual de materiais quando necessário.",
    ]))
    story.append(Spacer(1, 6))

    # 4. RESPONSABILIDADES DO ENCARREGADO
    story.append(secao_titulo("4. RESPONSABILIDADES DO ENCARREGADO"))
    story.append(Spacer(1, 8))
    story.append(Paragraph(
        "Além das responsabilidades comuns aos colaboradores, compete ao Encarregado de "
        "Produção:", style_corpo,
    ))
    story.append(lista_marcadores([
        "Orientar continuamente os colaboradores sobre segurança do trabalho;",
        "Exigir o uso correto dos EPIs;",
        "Impedir atividades inseguras;",
        "Comunicar imediatamente qualquer acidente ou incidente;",
        "Solicitar manutenção quando identificar riscos;",
        "Garantir que somente pessoas autorizadas operem equipamentos;",
        "Acompanhar o cumprimento das Ordens de Serviço;",
        "Zelar pela integridade física da equipe sob sua supervisão.",
    ]))
    story.append(Spacer(1, 6))
    ...
    story.append(Spacer(1, 8))
    story.append(Paragraph("É obrigatório:", style_corpo))
    story.append(lista_marcadores([
        "Cumprir todas as normas de segurança da empresa;",
        "Fiscalizar o uso correto dos EPIs;",
        "Não permitir a retirada de proteções das máquinas;",
        "Garantir que equipamentos defeituosos sejam retirados de operação;",
        "Comunicar imediatamente situações de risco;",
        "Manter as áreas de circulação desobstruídas;",
        "Participar dos treinamentos promovidos pela empresa;",
        "Orientar os colaboradores sempre que identificar desvios de segurança.",
    ]))
    story.append(Spacer(1, 6))
    ...
    story.append(Spacer(1, 8))
    story.append(Paragraph(
        "Durante as atividades é obrigatório utilizar, conforme aplicável:", style_corpo,
    ))
    story.append(lista_marcadores([
        "Botina de segurança;",
        "Óculos de segurança;",
    ...
    story.append(Spacer(1, 8))
    story.append(Paragraph("O Encarregado deverá:", style_corpo))
    story.append(lista_marcadores([
        "Suspender imediatamente as atividades em caso de risco grave e iminente;",
        "Acionar a gerência e os responsáveis pela emergência;",
        "Orientar a evacuação da área quando necessário;",
        "Isolar o local até que a situação seja controlada;",
        "Registrar e comunicar imediatamente qualquer ocorrência.",
    ]))
    story.append(Spacer(1, 6))
    ...
    story.append(lista_marcadores([
        "Recebi orientação sobre os riscos existentes na função de Encarregado de Produção;",
        "Recebi orientação quanto ao uso correto dos EPIs;",
        "Comprometo-me a cumprir e fazer cumprir as normas de segurança da empresa;",
        "Comunicarei imediatamente qualquer condição insegura;",
        "Estou ciente de que o descumprimento das normas poderá acarretar medidas "
    ...
    story.append(lista_marcadores([
        "Norma Regulamentadora nº 01 – Disposições Gerais e Gerenciamento de Riscos "
        "Ocupacionais;",
    ...
    story.append(Spacer(1, 8))
    story.append(Paragraph(
        "Declaro que recebi as orientações constantes nesta Ordem de Serviço, compreendi os "
        "riscos inerentes às atividades de Encarregado de Produção, fui instruído quanto ao "
        "uso correto dos Equipamentos de Proteção Individual (EPIs) e comprometo-me a "
        "cumprir e fazer cumprir integralmente as normas de segurança estabelecidas pela "
        "Fertiflora Fertilizantes Ltda.", style_corpo,
    ))
    story.append(Spacer(1, 8))
    ...
    assinaturas = Table(
        [[
            build_bloco_assinatura(
                "Colaborador",
                [("Nome", "Diego Ianch Correa"), ("Assinatura", None)],
            ),
            build_bloco_assinatura(
...
def footer(canvas, doc):
    ...
    canvas.drawString(20 * mm, 15 * mm, "FertiFlora Fertilizantes Ltda. — SST-OS-002 — Rev. 00")
    canvas.drawRightString(190 * mm, 15 * mm, f"Página {doc.page}")
    canvas.setFont("Helvetica", 6.5)
    canvas.drawCentredString(
        105 * mm, 10.5 * mm,
        "Documento interno da Fertiflora Fertilizantes Ltda. – Proibida a reprodução parcial "
        "sem autorização.",
    )
    canvas.drawCentredString(
        105 * mm, 7.5 * mm,
        "Revisão sempre que houver alteração de função, processo, riscos ocupacionais ou "
...
def main():
    doc = SimpleDocTemplate(
        ...
        title="Ordem de Serviço NR-01 - Encarregado de Produção",
